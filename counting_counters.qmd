...
```{python}
counts['dummy'] = 1
levels = counts.groupby(['submission_id', 'username'], sort=False).sum()
...
submission_id = effective_counters.idxmax().iat[0]
s = (f"The thread with the highest number of effective counters is "
     ...
     f"with {effective_counters.loc[submission_id].iat[0]:.1f} counters.")
Markdown(s)
